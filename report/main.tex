...
The ZYX Euler angle convention, while intuitive and practical for certain robotic systems, has a significant limitation: it becomes singular when \(\beta = \pm 90^\circ\), a phenomenon known as the \textit{gimbal lock}. At these angles, the pitch rotation aligns the global and local axes, effectively reducing the degrees of freedom in orientation from three to two. Mathematically, this occurs because \(\tan(\beta)\) and \(1/\cos(\beta)\) become undefined when \(\cos(\beta) = 0\), making the Jacobian matrix \(\mathbf{J}(\alpha, \beta)\) non-invertible.
